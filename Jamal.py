...
import sys,re,time,requests,colorama,string,random,bs4,tabulate,threading,base64,ctypes,json,prettytable,secrets,instagram_private_api
...
url = "https://youtube.com/@oranginisiald"
...
if not os.path.exists(flag_file_path):
    ...
    with open(flag_file_path, "w") as flag_file:
        flag_file.write("Pertanyaan Akan Di Jawab Melalui Dm Ig @oranginisiald")
...
def caramel_freya():
  autoketik(f"""
{U}╔╗ {N}╦  ╦   {Y}•{P} Creator{M} :{P}  DII X Vindra X Dell Store{P}
{U}╠╩╗{N}╚╗╔╝   {Y}•{N} Github{M}  :{B}  github.com/dii37{P}
{U}╚═╝{N} ╚╝{Y} @{H}4{Y} •{N} Youtube{M} :{P}  DIONE ID{P}
""")

def caramel_freya_2():
  autoketik(f"""
{U}╔╦╗{N}┌┬┐{Y}•{P} Creator{M} :{P}  DII X Dell Store{P}
{U} ║ {N} │ {Y}•{N} Github{M}  :{B}  github.com/dii37{P}
{U} ╩ {N} ┴ {Y}•{N} Youtube{M} :{P}  DIONE ID{P}
""")

def caramel_freya_3():
  autoketik(f"""
{U} ___{N} ___{Y}  •{P} Creator{M} :{P}  DII X Vindra ID{P}
{U}/ __|{N} _ ){Y} •{N} Github{M}  :{B}  github.com/dii37{P}
{U}\__ \{N} _ \{Y} •{N} Youtube{M} :{P}  DIONE ID{P}
{U}|___/{N}___/
""")

def caramel_freya_4():
  autoketik(f"""
{U}╔╦╗{N}┳┓{Y}•{P} Creator{M} :{P}  DII{P}
...
def main_menu_tt(): #Menu Awal tt
    try:
        ...
        if suer == "1":
            ...
            os.system("clear");caramel_freya_2();auth_list_tt()
        elif suer == "0":
            os.system("clear");caramel_freya();main_menu()
    except KeyboardInterrupt:
        sys.exit("\nTerminating the script.")
